...
class MultiObjectDataset(torch.utils.data.Dataset):
    """Synthetic dataset of scenes with multiple objects"""
    ...
    def _check_valid(self, index):
        ...
        # accept either RGB or RGBA pngs
        pngs = glob.glob(os.path.join(dir_path, "*.png"))
        if len(pngs) != self.n_views:
            return False
    ...
    def __getitem__(self, index):
        ...
        for frame in transform["frames"]:
            ...
            obj_path = os.path.join(dir_path, f"{basename}.png")
            ...
            if img.shape[-1] == 4:
                # real alpha mask
                mask_np = img[..., 3].astype(np.float32) / 255.0
            else:
                # RGB image → create full 1 mask
                mask_np = np.ones(img.shape[:2], dtype=np.float32)

            mask = self.mask_to_tensor(mask_np)

            # compute bbox (foreground region)
            rows = np.any(img[..., :3] != 255, axis=1)
            cols = np.any(img[..., :3] != 255, axis=0)
            rnz = np.where(rows)[0]
            cnz = np.where(cols)[0]
            ...
            img_tensor = img_tensor * mask + (1.0 - mask)
